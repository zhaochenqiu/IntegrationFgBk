	...
	while (!judge)
	{
		if (stackflag == 1)
		{
			...
			while (stackpos1 != 0)
			{
				...
				//		sigmas[i] = MAX(sum_sig[i]/imgcount, rate*mus[i]);
						sigmas[i] = MAX(sigmas[i],rate*mus[i]);

						if (ABS(mus[i] - value1) <= sigmas[i])
						{
							judgevalue ++;
						}

						sum_sig[i] = sum_sig[i] + ABS(mus[i] - value1);
						sigmas[i] = sum_sig[i]/imgcount;

					}

					if (judgevalue > threshold)
					{
						imgcount ++;
			...
			while (stackpos2 != 0)
			{
				...
				if (tempvalue == 0)
				{
					...
					//	sigmas[i] = MAX(sum_sig[i]/imgcount, rate*mus[i]);
						...
						if (ABS(mus[i] - value2) <= sigmas[i])
						{
							judgevalue ++;
						}

						sum_sig[i] = sum_sig[i] + ABS(mus[i] - value2);
						sigmas[i] = sum_sig[i]/imgcount;
						//sigmas[i]	= sum_sig[i]/imgcount;
					...
					if (judgevalue > threshold)
					{
						...
						for (i = 0;i != 3;i ++)
						{
							...
							//sum[i] = sum[i] + value2;
							sum_mus[i] = sum_mus[i] + value2;
						}

						for (i = 0;i != 3;i ++)
						{
							//mus[i] = sum[i]/(double)imgcount;
							// sigmas[i]	= sum_sig[i]/imgcount;
							mus[i]		= sum_mus[i]/imgcount;
						}
