function re_map = getMapRange_multi_model2img(grayimg_cur,modelimg,label_list,H_list,label_img,range,H_error)
    ...
    for j = 1:column_img
        ...
        if posx > 0 & posx < column_img + 1 & posy > 0 & posy < row_img + 1
            ...
            if (re_map(i,j,1) == -1)
                ...
                oldmapx = re_map(i,j,1);
                oldmapy = re_map(i,j,2);

                oldsub = abs(value - grayimg_cur(oldmapy,oldmapx));
                newsub = abs(value - grayimg_cur(mapy,mapx));
