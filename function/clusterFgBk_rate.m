...
[row_t column_t] = size(segment_matrix_rate);
count = 0;
...
for i = 1:row_t
    value = segment_matrix_rate(i,2);

    for  j = 1:value
        segmentrate = segment_matrix_rate(i,1);
...
%        [bklabels_img bknumlabels] = segmentImg_SLIC_rand_msk(uint8(img),segmentnums,bkmask,num_factor,num_scale);
        ...
        [fglabels_img fgnumlabels] = segmentImg_SLIC_rand_msk_rate(uint8(img),segmentrate,fgmask,num_factor,num_scale);

%        [bklabels_img bknumlabels] = segmentImg_SLIC_rand_msk_rate(uint8(img),segmentrate,bkmask,num_factor,num_scale);
