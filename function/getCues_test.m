function [re_fgcues re_bkcues re_H re_model re_model_fg_up] = getCues_model2img_img2model_bk(img_cur,model)
...
map_img2model = getMap_tar2src(grayimg_cur,bkimg,store_list,store_transform,labels_img,range_single,H_full);

mus_fg      = aligMap_src2tar(mus,map_img2model);
sigmas_fg   = aligMap_src2tar(sigmas,map_img2model);
weights_fg  = aligMap_src2tar(weights,map_img2model);
...
%{
map_img2model = getMap_tar2src(grayimg_cur,bkimg,store_list,store_transform,labels_img,0,H_full);

mus_fg_up      = aligMap_src2tar(mus,map_img2model);
sigmas_fg_up   = aligMap_src2tar(sigmas,map_img2model);
weights_fg_up  = aligMap_src2tar(weights,map_img2model);

[mus_fg_up sigmas_fg_up weights_fg_up] = reInitiaModel(img_cur,mus_fg_up,sigmas_fg_up,weights_fg_up);
%}
...
[compare_fg_up fgimg_fg_up similary_fg_up] = getFgImage(grayimg_cur,mus_fg_up,sigmas_fg_up,weights_fg_up,threshold_gmm);

[mus_fg_up sigmas_fg_up weights_fg_up] = updateBkImage(grayimg_cur,mus_fg_up,sigmas_fg_up,weights_fg_up,compare_fg_up);
%}
