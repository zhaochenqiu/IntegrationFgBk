function [re_fgcues re_bkcues re_H re_model re_model_up] = getCues_model2img_img2model_bk(img_cur,model)
...
map_img2model = getMap_tar2src(bkimg,grayimg_cur,store_list,store_transform,labels_img,0,H_full);

mus_fg_up      = aligMap_tar2src(mus,map_img2model);
sigmas_fg_up   = aligMap_tar2src(sigmas,map_img2model);
weights_fg_up  = aligMap_tar2src(weights,map_img2model);
...
re_model_up = {mus_fg_up,sigmas_fg_up,weights_fg_up};
